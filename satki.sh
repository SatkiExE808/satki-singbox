...
echo "Choose TLS certificate method:"
echo "1) Let's Encrypt (automated, get from Let's Encrypt)"
echo "2) Self-signed (auto-generated by script)"
...
# Function to select and ensure the right certificate
ensure_cert() {
    case "$TLS_MODE" in
        letsencrypt)
            ...
            if [[ -f "$LE_CERT_PATH" && -f "$LE_KEY_PATH" ]]; then
                ...
                echo "Let's Encrypt certificate not found or failed to obtain. Falling back to self-signed."
                generate_selfsigned_cert
            fi
            ;;
        selfsigned|*)
